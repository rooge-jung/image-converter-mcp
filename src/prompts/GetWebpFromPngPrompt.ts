...
interface GetWebpFromPngInput {
  /** Base64로 인코딩된 PNG 이미지 데이터 */
  pngImageData: string;
  /** WebP 품질 (1-100, 기본값: 80) */
  quality?: number;
  /** 무손실 압축 사용 여부 (기본값: true) */
  lossless?: boolean;
  /** 애니메이션 지원 여부 (기본값: false) */
  animated?: boolean;
  /** 변환된 이미지를 다운로드할지 여부 (기본값: false) */
  download?: boolean;
}

class GetWebpFromPngPrompt extends MCPPrompt<GetWebpFromPngInput> {
  name = "get_webp_from_png";
  description =
    "PNG 이미지를 WebP 형식으로 변환하는 프롬프트입니다. 이미지 데이터와 변환 옵션을 제공하세요.";
